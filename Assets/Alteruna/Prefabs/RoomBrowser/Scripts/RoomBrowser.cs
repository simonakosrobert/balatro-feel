...
namespace Alteruna
{
	public class RoomBrowser : CommunicationBridge
	{
		...
		[SerializeField] private Button _buttonDirectJoin;
		[SerializeField] private TMP_InputField yourNameInput;
		[SerializeField] private TMP_Text yourName;
		...
		{
			StartCoroutine(Utility.UIElementSwitchEnum(new GameObject[]{SideMenu, RoomList}, new GameObject[]{CreateRoomMenuObj}, touchBlock));
		}

		public void CreateRoomBackButton()
		{
			StartCoroutine(Utility.UIElementSwitchEnum(new GameObject[]{CreateRoomMenuObj}, new GameObject[]{SideMenu, RoomList}, touchBlock, 5, false, false));
		}

		public void CreateRoomSubmit()
		{
			StartCoroutine(Utility.UIElementSwitchEnum(new GameObject[]{CreateRoomMenuObj}, new GameObject[]{Lobby}, touchBlock));
		}

		// public static void UISwitcher(GameObject[] closeList, GameObject[] openList, bool closeDirectionDown = true, bool openDirectionDown = true)
		// {
		// 	StartCoroutine(UIElementSwitchEnum(closeList, openList, closeDirectionDown, openDirectionDown));
		// }
		...
		{	
			...
			_buttonDirectJoin.interactable = false;
			...
			yourNameInput = GameObject.Find("Your name").GetComponent<TMP_InputField>();
			yourName = GameObject.Find("Your name title").GetComponent<TMP_Text>();
		...
		public void ConnectNameChange()
		{	
			if (yourNameInput.text != "")
			{	

				string name = yourNameInput.text.Replace(" ", "_");

				Multiplayer.SetUsername(name);
				Multiplayer.Connect();
				yourNameInput.interactable = false;
				GameObject.Find("button Connect").GetComponent<Button>().interactable = false;
				pressedConnect = true;
			}
		}
		...
		private void Update()
		{
			...
			if (!Multiplayer.IsConnected && pressedConnect) yourName.text = "Connecting...";
			else if (Multiplayer.IsConnected && pressedConnect)
			{
				...
				yourName.text = $"Welcome,\n{yourNameInput.text.Replace(" ", "_")}";
				GameObject.Find("button Connect").SetActive(false);
				yourNameInput.transform.gameObject.SetActive(false);
				_buttonCreate.interactable = true;
				_buttonDirectJoin.interactable = true;
				pressedConnect = false;
			}
			//Gameplayinfo = GameObject.Find("GameplayInfo");
			//Gameplayinfo.GetComponent<GameplayInfo>().myName = yourNameInput.text;
			//if (yourNameInput.text != "") Multiplayer.SetUsername(yourNameInput.text);
			//Debug.Log(Multiplayer.Me.Name);
		}
